...
const updateVersionInPackageJSON = (filePath: string) => {
    ...
    if (oldVersion === version) return console.log(`${colors.gray}${truncatedPath}${colors.reset}: ${colors.yellow}same version${colors.reset}, skipping...`);
    if (oldVersion === undefined) return console.log(`${colors.gray}${truncatedPath}${colors.reset}: ${colors.red}version not found${colors.reset}, skipping...`);

    packageJSON.version = version;
    fs.writeFileSync(filePath, JSON.stringify(packageJSON, null, 2), 'utf-8');

    console.log(`${colors.gray}${truncatedPath}${colors.reset}: ${colors.green}${oldVersion}${colors.reset} -> ${colors.cyan}${version}${colors.reset}`);
};

const updateVersionInAllPackages = () => {
    const packageJSONFiles = fs.readdirSync(packagesDir).map((file) => path.join(packagesDir, file, 'package.json'));
